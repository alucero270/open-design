...
export async function startServer({
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    let pendingLinkedRepoProbeSummary = null;
    let terminalErrorAfterLinkedRepoChangesPromise = null;
    const emitLinkedRepoChangesForRun = (summary) => {
      design.runs.setRepoChanges(run, summary);
      send('repo_changes', summary);
    };
    const emitPendingLinkedRepoProbeSummary = () => {
      if (!pendingLinkedRepoProbeSummary) return;
      const summary = pendingLinkedRepoProbeSummary;
      pendingLinkedRepoProbeSummary = null;
      emitLinkedRepoChangesForRun(summary);
    };
    const resetLinkedRepoCaptureStateForRetry = () => {
      ...
      design.runs.setRepoChanges(run, null);
    };
    ...
    const finishWithRetryDecision = (status, code = null, signal = null) => {
      ...
      emitPendingLinkedRepoProbeSummary();
      ...
      if (!linkedRepoChangesCapturePromise) {
        ...
        linkedRepoChangesCapturePromise = (async () => {
          try {
            ...
            const hasRetryBlockingRepoSignal = isRepoChangesRetryBlocking(summary);
            const hasRelevantRepoSignal =
              hasRetryBlockingRepoSignal ||
            ...
            if (hasRelevantRepoSignal && attemptIdAtStart === linkedRepoCaptureAttemptId) {
              if (hasRetryBlockingRepoSignal) {
                emitLinkedRepoChangesForRun(summary);
              } else {
                pendingLinkedRepoProbeSummary = summary;
              }
            }
          } catch (err) {
            console.warn(
    ...
    child.on('close', async (code, signal) => {
      ...
      if (agentStreamError) {
        ...
        return finishClosedRun('failed', code === 0 ? 1 : (code ?? 1), signal ?? null);
      }
      if (
        code !== 0 &&
        !run.cancelRequested
